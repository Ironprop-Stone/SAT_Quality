...
void Solver::assign_emb_path(string filename) {
  cout << "Read embedding from: " << filename << endl;
  int no_nodes, no_dim; 
  vector<float> line; 
  float val;

  ifstream emb_file;
  emb_file.open(filename, ios::in);
  emb_file >> no_nodes >> no_dim;
  cout<<"no_nodes: "<<no_nodes<<", no_dim: "<<no_dim<<endl;
  for (int i = 0; i < no_nodes; i++) {
    ...
    internal->emb.push_back(line);
    internal->emb_used.push_back(false);
  }
  emb_file.close();
}
