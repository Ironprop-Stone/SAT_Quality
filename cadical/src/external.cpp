...
int External::solve (bool preprocess_only) {
  ...
  int start_clause_cnt = internal->clauses.size(); // Stone: Get the number of clauses before solving
  int res = internal->solve (preprocess_only);
  check_solve_result (res);
  reset_limits ();


  // Stone: Output the final clauses 
  int end_clause_cnt = internal->clauses.size();

  for (int clause_idx = start_clause_cnt; clause_idx < end_clause_cnt; clause_idx++) {
    cout << "Learnt: " << internal->clauses[clause_idx]->size << ": ";
    for (int i=0; i<internal->clauses[clause_idx]->size; i++) {
      if (i == internal->clauses[clause_idx]->size - 1) {
        cout << internal->clauses[clause_idx]->literals[i] << endl;
      }
      else {
        cout << internal->clauses[clause_idx]->literals[i] << ", ";
      }
    }
  }
  return res;
